...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    relocate: {
      // Files to bundle up. Can be copied into same directory structure
      //files: ['*.html'],
      // or relocated into a new one:
      files: {'*.html': '',
              'dist/release/*.css': 'assets/css/',
              'dist/release/*.js': 'assets/js/libs'
             },
      // Folders to bundle up. Copy files and folders:
      //folders: ['things'],
      // or relocate them:
      //folders: {'things': 'thangs'},
      // all locations are relative to the destination parameter.
  ...
  grunt.registerTask("relocate", "Relocate files and directories.", function(){
    ...
    function relocateToPath(the_list){
      _.each(the_list, function(newpath, path, l){
        ...
        destination = destination.replace(/\/+/g, '/');
        grunt.log.writeln('relocate ' + path + ' to ' + destination);
  ...
  grunt.registerTask("package", "release relocate");
